...
import { Bot, Brain, Package, BarChart3, Clock, Plug, ArrowRight, Zap, TrendingDown, Scaling, Shield, Cpu, Boxes, Truck } from "lucide-react";
...
const hermitPricing = [
  { name: "Hermit Pilot", price: 9999, yearlyPrice: 99999, features: ["Single dark store", "AI demand forecasting", "Basic automation", "Standard support", "Monthly reporting"] },
  { name: "Hermit Scale", price: 24999, yearlyPrice: 249999, popular: true, features: ["Up to 5 dark stores", "Full automation suite", "Advanced AI & ML", "Priority support", "Real-time analytics", "Custom integrations"] },
  { name: "Hermit Unlimited", price: 49999, yearlyPrice: 499999, features: ["Unlimited dark stores", "Edge computing network", "Dedicated AI team", "24/7 support", "SLA guarantee", "On-premise deployment"] },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <Sparkles className="w-full h-full" density={80} size={1.5} speed={0.3} color="hsl(217 91% 60%)" opacity={0.2} />
        </div>
        ...
        <div className="section-container relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <span className="inline-block px-4 py-1.5 rounded-full border border-primary/30 bg-primary/10 text-sm text-primary font-semibold mb-6">Dequeue Hermit</span>
              <h1 className="text-5xl lg:text-7xl font-bold font-heading mb-6 leading-[1.05]">
                Autonomous <span className="gradient-text">Retail Infrastructure.</span>
              </h1>
        ...
        <div className="section-container">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="liquid-glass-card p-8 text-center relative overflow-hidden">
      ...
      <section className="py-24 page-section" ref={capRef}>
        <div className="section-container">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={capInView ? { opacity: 1, y: 0 } : {}} className="text-center max-w-3xl mx-auto mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full border border-border bg-secondary text-sm text-muted-foreground mb-6">Capabilities</span>
            <h2 className="text-4xl lg:text-5xl font-bold font-heading">Full-Stack Autonomy</h2>
          </motion.div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {capabilities.map((c, i) => (
              <motion.div key={c.title} initial={{ opacity: 0, y: 30 }} animate={capInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.1 }} className="liquid-glass-card p-6 group hover:border-primary/30 transition-all duration-500">
      ...
      <section className="py-24 page-section">
        <div className="section-container">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading mb-6">Built on Advanced AI</h2>
          </motion.div>
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {techPillars.map((tp, i) => (
              <motion.div key={tp.title} initial={{ opacity: 0, x: i % 2 === 0 ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="liquid-glass-card p-6 flex items-start gap-4">
      ...
      <section className="py-24 page-section relative overflow-hidden" ref={impactRef}>
        ...
        <div className="section-container relative">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {impact.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, scale: 0.8 }} animate={impactInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay: i * 0.1, type: "spring" }} className="liquid-glass-card p-8 text-center">
